...
class UpperPcWin(QtWidgets.QMainWindow,Ui_MainWindow):  # 主窗口只负责处理左侧按钮弹出窗口的逻辑
    ...
    def loadPersistedAddedPowers(self):
        ...
        for item in power_items:
            ...
            self.added_power_widgets.append(widget_obj)
    ...
    def savePersistedAddedPowers(self):
        ...
        power_items = []
        for widget_obj in self.added_power_widgets:
    ...
    def delBTN(self):
        ...
        DelBtnCustom.clicked.connect(self.removeLastAddedPower)
    ...
    def removeLastAddedPower(self):
        if not self.added_power_widgets:
            ...
            return
        self.DelSubWin(self.added_power_widgets[-1])
    ...
    def showPowerContextMenu(self, widgetObj, button, pos):
        ...
        if widgetObj not in self.added_power_widgets:
    ...
    def AddSubWin(self,widgetObj, show_controls=True):
        ...
        if show_controls:
            ...
            is_resizable_page = isinstance(widgetObj, MUNPower)
            scroll_area.setWidgetResizable(is_resizable_page)
            scroll_area.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarAsNeeded)
            scroll_area.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarAsNeeded)
            scroll_area.setAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignTop)
            if is_resizable_page:
    ...
    def DelSubWin(self,widgetObj):
        ...
        # 删除页面对应对象
        if widgetObj:
            if widgetObj in self.added_power_widgets:
                self.added_power_widgets.remove(widgetObj)
